...
class WhisperCLI:
    ...
    def _select_microphone(self) -> None:
        ...
        if self.headless:
            audio_service.input_route = "system"
            return

        print("\n🎤 Available input devices:")
        for row, choice in enumerate(choices):
            marker = " (SELECTED)" if choice.key == audio_service.input_route else ""
            print(f"  {row}: {choice.label}{marker}")

        while True:
            try:
                choice = input(
                    f"\nSelect microphone (0-{len(choices)-1}, or press ENTER for default): "
                ).strip()
                if choice == "":
                    audio_service.input_route = "system"
                    print("🎯 Following the system default input")
                    return

                idx = int(choice)
                if 0 <= idx < len(choices):
                    audio_service.input_route = choices[idx].key
                    print(f"🎯 Selected: {choices[idx].label}")
                    return
